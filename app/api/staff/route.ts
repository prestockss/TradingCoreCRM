...
type DbRole = 'admin' | 'manager' | 'agent';
...
async function authorize(request: Request) {
  ...
  return { admin, user, role: profile.role as DbRole };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const role: DbRole = body.role === 'admin' ? 'manager' : 'agent';
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (body.action === 'password') {
      ...
      const { data: ownerCheck } = await auth.admin.from('profiles').select('login_id').eq('id', id).maybeSingle();
      if (ownerCheck?.login_id === 'prestockss') throw new Error('대표계정은 비활성화할 수 없습니다.');
